...
- **Critère 7 du lot 1a — isolation entre comptes — REPORTÉ, non vérifié.** Le compte Postmark est en attente d'approbation et n'autorise l'envoi qu'entre adresses d'un même domaine : impossible de créer un second compte, donc impossible de vérifier qu'un utilisateur ne voit pas les projets d'une autre organisation. Les dix autres critères sont validés. **À faire dès l'approbation du compte Postmark**, avant le lot 1b si elle arrive d'ici là — c'est le contrôle d'accès du produit qui reste sans preuve d'exécution.
- **Prochain : lot 1b — membres.**
...
- **Le `request_id` n'est pas propagé dans `lib/orgs.ts`** : le logger y est créé avec son propre `crypto.randomUUID()`, si bien que `orgs.ensure.failed` et `orgs.ensure.created` ne peuvent être rapprochés d'aucune autre ligne de la même requête — or c'est précisément ce rapprochement que la journalisation structurée existe pour permettre. **À corriger** : faire descendre le logger de la requête jusqu'à `ensureOrganizationForUser`, au lieu d'en fabriquer un sur place.
