...
Mémoire du projet entre les sessions. Chargé à chaque session Claude Code. `DESIGN.md`, à la racine, ne l'est pas : toute règle qui doit survivre à un `/clear` vit ici. `PASSATION-Omnisc.md`, à la racine, ne l'est pas non plus : c'est le point d'entrée d'une session vierge — il ne contient aucune règle, seulement où lire et où en est le chantier.
...
- **Lot 1a — auth, organisations, projets : terminé le 2 septembre 2026, vérifié en production.** `lib/auth/session.ts` (`getUser` / `requireUser`, jamais `getSession`), `lib/supabase/auth.ts` (clé `anon`, `auth.*` seul), `middleware.ts` (rafraîchissement du jeton, aucune décision d'accès), `app/(auth)/login/`, `app/auth/callback/`, `app/auth/logout/`, `lib/orgs.ts` (organisation créée à la volée, sans écran), `lib/entitlements.ts` (`canCreateProject`, seul point de décision), `lib/projects/queries.ts` (appartenance vérifiée dans le SQL, `null` et non 403) et `lib/projects/inbound-address.ts`, `app/projects/`, `app/p/[slug]/`, `supabase/migrations/0002_auth_projects.sql` (`organization_members`, `projects.name`, suppression de `projects.slug`). Deux tranchages reportés en section D : `organization_members` distincte de `memberships`, et la frontière préfixe/suffixe de l'adresse de projet. Ce que la vérification a appris est consigné en section B, « Constats de production ».
- **Critère 7 du lot 1a — isolation entre comptes : VÉRIFIÉ le 3 septembre 2026.** Un utilisateur ne voit ni les projets ni l'organisation d'un autre compte, et une URL de projet d'une autre organisation renvoie une page introuvable. Les onze critères du lot 1a sont validés.
- **Le compte Postmark a été approuvé le 3 septembre 2026.** La restriction qui n'autorisait l'envoi qu'entre adresses d'un même domaine n'existe plus. C'est elle qui avait empêché de créer un second compte de test et fait reporter le critère 7 ; les deux constats qui en découlaient sont caducs et ont été retirés.
- **Lot 1b — membres : invitations par lien, rôles, écran d'organisation. Déployé le 3 septembre 2026 ; VÉRIFICATION EN PRODUCTION EN COURS.** `supabase/migrations/0003_invitations.sql` (`organization_invitations`, index unique partiel des invitations en attente, index `owner` unique par organisation), `lib/members/` (`token.ts` et `queries.ts` : organisation possédée, membres, invitation, révocation, retrait, acceptation), `lib/http/origin.ts` (origine et chemin de retour assainis, partagés avec le lien magique), `app/organization/`, `app/invite/[token]/`, `lib/orgs.ts` corrigé sur `role = 'owner'`, `lib/projects/queries.ts` en deux natures, `lib/auth/signup-allowlist.ts` ouvert aux invités. **Tant que la liste d'acceptation n'est pas passée en production, ce lot n'est pas terminé** (section B, « Fin de lot »).
- **Prochain : lot 1c — connexion Google.** Retirée du lot 1b : elle ne partage aucun fichier avec les membres, et son risque est une configuration externe sur la console Google Cloud, pas du code.
...
**Tout utilisateur possède toujours sa propre organisation**, créée à la première connexion, y compris s'il arrive par une invitation. Accepter une invitation AJOUTE une appartenance, cela ne remplace jamais l'organisation personnelle. Conséquence sur le code : `ensureOrganizationForUser` cherche une appartenance `owner`, jamais « une appartenance quelconque » — sinon un invité passerait pour déjà pourvu et ne pourrait jamais créer de projet.

**Une organisation n'a qu'un seul `owner` en V1.** « Mon organisation » se définit comme celle où l'utilisateur est `owner`, et il y en a exactement une : d'où un écran `/organization` sans paramètre de route ni sélecteur. L'invariant est matérialisé en base par l'index unique partiel `organization_members_single_owner_idx`. Le jour où deux associés partageront un abonnement, il devra être levé EXPLICITEMENT, par une migration qui supprime cet index — pas contourné au cas par cas.

**Une invitation est de portée ORGANISATION**, jamais projet : elle donne accès à tous les projets de cette organisation, présents et futurs. C'est la conséquence directe du périmètre d'accès unique ci-dessus. Tant qu'une organisation n'a qu'un projet — le défaut, `max_active_projects = 1` — la différence est nulle ; elle deviendra visible au premier client à deux projets, et c'est alors que `memberships` en portée projet prendra son sens. **Conséquence obligatoire pour l'interface : tout libellé d'invitation nomme l'organisation, jamais un projet.** « Ajouter un membre à l'organisation », jamais « inviter sur ce projet ». Un bouton qui annonce l'accès à un projet et donne l'accès à toute l'organisation est un défaut, pas une approximation.

**`canCreateProject(orgId)` n'est appelée que sur l'organisation dont l'utilisateur est `owner`.** Le quota d'une organisation n'est jamais consommé par un invité : `createProjectForUser` résout l'organisation par `role = 'owner'` avant toute autre chose.
...
**Constats de production.** Relevés aux vérifications des lots 0a (31 août 2026) et 1a (2 septembre 2026).
...
- **Un Site URL Supabase saisi en `http://` produit un `ERR_CONNECTION_RESET` côté Vercel**, que l'on prend pour un blocage réseau local : Vercel n'accepte pas le trafic en clair et coupe la connexion avant toute réponse, donc il n'y a ni page d'erreur ni code de statut à lire. Le lien magique était injoignable depuis un poste et s'ouvrait depuis un téléphone en 4G, ce qui a envoyé le diagnostic une session entière dans la mauvaise direction. **Toute URL saisie dans Supabase est en `https://`.**
- **Un `redirect_to` réduit à l'hôte nu, sans chemin, est la signature du repli de GoTrue sur le Site URL** quand l'`emailRedirectTo` demandé ne correspond à aucun motif de l'allowlist. Le repli est SILENCIEUX : aucune erreur, aucun avertissement, seulement un lien qui n'atterrit pas où le code l'a demandé. Un `emailRedirectTo` en `http://` ne correspond pas à un motif en `https://` — c'est ainsi que le constat précédent produisait aussi la perte du chemin. **Les motifs de Redirect URLs doivent couvrir à la fois les alias de branche et les hôtes par déploiement, et se terminer par `/**`.** Le protocole du lien n'est plus lu dans `x-forwarded-proto` dès que `VERCEL_ENV` vaut `preview` ou `production` : sur un déploiement il vaut `https` par construction. L'origine calculée est journalisée avant l'envoi, sous `auth.magic_link.redirect_requested` — sans cette trace, départager une origine fausse d'un repli de GoTrue demande de décortiquer un e-mail reçu.
- **Une variable Vercel marquée « Sensitive » ne peut plus JAMAIS être relue**, ni par `vercel env pull`, ni par l'interface : elle revient en `[SENSITIVE]`. On ne vérifie donc pas la valeur d'une telle variable — **on l'écrase et on contrôle l'effet côté base**. Conséquence à ne pas oublier au diagnostic : constater que « les variables sont bonnes » après un `env pull` ne prouve rien pour `DATABASE_URL` ni `SUPABASE_URL`, et laisse entière l'hypothèse d'un déploiement qui authentifie contre un projet Supabase et écrit dans un autre.
- **Le `request_id` n'était pas propagé dans `lib/orgs.ts`** : le logger y était créé avec son propre `crypto.randomUUID()`, si bien que `orgs.ensure.failed` et `orgs.ensure.created` ne pouvaient être rapprochés d'aucune autre ligne de la même requête — or c'est précisément ce rapprochement que la journalisation structurée existe pour permettre. **CORRIGÉ au lot 1b.** `lib/logger.ts` expose `loggerForHeaders(await headers(), …)`, et c'est la seule façon correcte d'obtenir un logger hors d'un handler de route : **ne jamais écrire `createLogger(crypto.randomUUID())` dans du code de requête.**
...
- **`organization_members` et `memberships` sont deux tables distinctes, et les confondre casserait l'une des deux** (tranché au lot 1a). `organization_members` répond à « qui possède l'abonnement » : la ligne existe dès la première connexion, avant qu'aucun projet n'existe, et son `user_id` est **NOT NULL** — on n'appartient pas à une organisation sans compte. `memberships`, de portée projet, répond à « qui peut lire ce projet », y compris quelqu'un qui n'a pas encore de compte. **Elle n'a PAS été créée au lot 1b** : le lot 1b invite à l'échelle de l'organisation, ce qui suffit tant qu'une organisation n'a qu'un projet, et `memberships.identity_id` référence `identities`, qui n'existe pas avant le lot 3a. Elle sera créée au plus tôt au lot 3a, avec la table qu'elle référence.
...
- **`organization_invitations` (lot 1b) — l'invitation ne crée AUCUN compte.** Elle pose une ligne portant un jeton ; la personne suit le lien, se connecte par lien magique — le chemin d'inscription normal, inchangé — et `organization_members` n'est écrite qu'à ce moment. Ne jamais pré-créer l'utilisateur par l'API admin de Supabase : chaque invitation non acceptée laisserait un compte orphelin. Une invitation donne toujours `member`, jamais `owner`, et le `check (role = 'member')` fige la règle en base. L'index unique partiel `(org_id, lower(email))` filtré sur `accepted_at is null and revoked_at is null` interdit deux invitations simultanées pour la même adresse tout en autorisant à réinviter quelqu'un qu'on a révoqué.
- **Le jeton d'invitation est stocké EN CLAIR, et c'est un choix, pas un oubli.** Aucun e-mail ne part avant le lot 5 : le propriétaire copie le lien depuis `/organization` et le transmet par sa propre messagerie, éventuellement plusieurs jours après l'avoir créé. Un stockage haché interdirait de le réafficher. Le jeton n'est pas un mot de passe — il ne donne qu'une chose, devenir `member` d'une organisation — il expire à 14 jours, il est révocable, et l'acceptation exige en plus que l'adresse authentifiée soit l'adresse invitée. **Ne pas le remplacer par une empreinte.**
- **Il n'existe aucune dette sur `projects_inbound_local_part_lower_idx`** : la faute de frappe qui a circulé (`projectfs_…`) ne correspond à rien, l'index porte bien ce nom sur les deux projets. Il fait doublon apparent avec `projects_inbound_local_part_key`, mais comme rien en base ne force la minuscule, c'est LUI qui porte la garantie que l'ingestion et `/p/[slug]` résolvent le même projet. **Aucune action.**
